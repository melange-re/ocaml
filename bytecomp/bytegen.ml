...
let rec comp_expr env exp sz cont =
  ...
  | Lsend(kind, met, obj, args, _) ->
      ...
      let getmethod, args' =
        ...
        match met with
          Lconst(Const_base(Const_int n, _)) -> (Kgetpubmet n, obj::args)
      ...
      if List.for_all (function (_, Lfunction _) -> true | _ -> false)
        ...
        let rec comp_init new_env sz = function
          ...
          | (id, _exp, RHS_floatblock blocksize) :: rem ->
              Kconst(Const_base(Const_int blocksize, default_pointer_info)) ::
          ...
          | (id, _exp, RHS_infix { blocksize; offset }) :: rem ->
              Kconst(Const_base(Const_int offset, default_pointer_info)) ::
              Kpush ::
              Kconst(Const_base(Const_int blocksize, default_pointer_info)) ::
              Kccall("caml_alloc_dummy_infix", 2) :: Kpush ::
              comp_init (add_var id (sz+1) new_env) (sz+1) rem
          | (id, _exp, RHS_function (blocksize,arity)) :: rem ->
              Kconst(Const_base(Const_int arity, default_pointer_info)) ::
              Kpush ::
              Kconst(Const_base(Const_int blocksize, default_pointer_info)) ::
              Kccall("caml_alloc_dummy_function", 2) :: Kpush ::
              comp_init (add_var id (sz+1) new_env) (sz+1) rem
          | (id, _exp, RHS_nonrec) :: rem ->
              Kconst(Const_base(Const_int 0, default_pointer_info)) :: Kpush ::
              comp_init (add_var id (sz+1) new_env) (sz+1) rem
  ...
  | Lprim(Paddint, [arg; Lconst(Const_base(Const_int n, _))], _)
    when is_immed n ->
      comp_expr env arg sz (Koffsetint n :: cont)
  | Lprim(Psubint, [arg; Lconst(Const_base(Const_int n, _))], _)
    when is_immed (-n) ->
      comp_expr env arg sz (Koffsetint (-n) :: cont)
  | Lprim (Poffsetint n, [arg], _)
    when not (is_immed n) ->
      comp_expr env arg sz
        (Kpush::
         Kconst (Const_base (Const_int n, default_pointer_info))::
         Kaddint::cont)
...
(* Integer first for enabling further optimization (cf. emitcode.ml)  *)
  ...
  | Lprim(Pmakeblock(tag, _tag_info, _mut, _), args, loc) ->
      let cont = add_pseudo_event loc !compunit_name cont in
...
let comp_block env exp sz cont =
  ...
  if used_safe > Config.stack_threshold then
    Kconst(Const_base(Const_int used_safe, default_pointer_info)) ::
